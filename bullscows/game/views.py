...
def bullscows(request):
    ...
    if player.game.count() == 0:
        creating_new_game(player)
        return render(request, 'game/bullscows.html', context={"new_game": 'Новая игра'})
    request_value = request.POST.get('value')
    data = request_value_handler(game=get_game(player), request_value=request_value)
